...
function getreservaTimeOutNotification(date) {
  return new Promise(resolve => {
    var sql="select u.tipoUsuario,t.usuario, r.idComputadora,r.idLaboratorio,JSON_ARRAYAGG(t.idToken) as tokenNoti from ReservaComputadora r, TokenNotification t,Usuario u where r.fin=? and (r.estado='En espera' or r.estado='En uso') and r.idUsuario=t.usuario and u.id=r.idUsuario group by t.usuario"
